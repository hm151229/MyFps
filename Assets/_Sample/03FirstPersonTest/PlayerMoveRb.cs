...
namespace MyFps
{
    public class PlayerMoveRb : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            {
                if (_input.Move != Vector2.zero)    //이동중
                {
                    rb.linearDamping = 5f;
                }
                else //멈추어 있을 때
...
리지디바디
- 다이나믹 리지디바디 (키네틱x)
:내가 상대 오브젝트에게 물리 영향을 주고 상대 오브젝트도 나에게 영향을 준다
...
ForceMode
ForceMode.Force (연속적인 힘, 질량 고려)
- 바람, 자기력 
ForceMode.Acceleration (연속적인 힘, 질량 무시)
-중력, 질량에 상관없이 일정한 가속을 구현할 때
ForceMode.Impulse (불연속적인 힘(일회성), 질량 고려)
-폭발, 타격, 순간적으로 적용하는 힘
ForceMode.VelocityChange (불연속적인 힘(일회성, 질량 무시)
